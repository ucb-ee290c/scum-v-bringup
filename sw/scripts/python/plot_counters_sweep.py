...
file_path_idac = "./logs/counter_regs_96m_10m_dcoff_0.3_idac_sweep_128.txt"
...
fig, axs = plt.subplots(len(cnt_n_sections), 1, figsize=(10, 4 * len(cnt_n_sections)), squeeze=False)
...
for i, (idac, cnt_n) in enumerate(cnt_n_sections.items()):
    ...
    axs[i, 0].plot(cnt_p, label=f"COUNTER_P, IDAC: {idac}", color="blue")
    axs[i, 0].plot(cnt_n, label=f"COUNTER_N, IDAC: {idac}", color="green")
    axs[i, 0].plot(cnt_p_minus_n, label=f"COUNTER_P - COUNTER_N, IDAC: {idac}", color="red")
