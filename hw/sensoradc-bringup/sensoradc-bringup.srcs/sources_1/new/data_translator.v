module data_translator #(
    ...
    assign fifo_rd_en = !fifo_empty && (!data_out_valid_reg || data_out_ready);
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            if (fifo_rd_en) begin
                case (byte_select)
                    2'b00: data_out_reg <= fifo_dout[OUTPUT_WIDTH-1:0];
                    2'b01: data_out_reg <= fifo_dout[2*OUTPUT_WIDTH-1:OUTPUT_WIDTH];
                    2'b10: begin
                        data_out_reg <= {{(OUTPUT_WIDTH-4){1'b0}}, fifo_dout[INPUT_WIDTH-1:2*OUTPUT_WIDTH]};
                        send_marker <= 1'b1;
                    end
